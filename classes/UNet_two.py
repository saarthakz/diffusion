...
class UNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.dim = dim
        out_channels = out_channels if out_channels != None else num_channels
        ...
        self.down_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    nn.Conv2d(num_channels, dim, kernel_size=3, padding=1)
                )
            ]
        )
        down_block_chans = [dim]
        ch = dim
        ds = 1
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                layers = [ResidualBlock(ch, mult * dim, time_embed_dim, dropout)]
                ch = mult * dim
